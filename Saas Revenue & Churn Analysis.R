...
Revenue <- Revenue %>%
  mutate(year_month = month,
         month = sapply(strsplit(month,"-"),`[`,2)) 

Revenue %>%
  group_by(month) %>%
  summarize(Monthly_Total = sum(total_mrr,na.rm = TRUE),.groups = "drop") %>% 
  ggplot(aes(x = month, y = Monthly_Total,group = 1)) +
  geom_line() +
  ggtitle("Monthly Total MMR")
